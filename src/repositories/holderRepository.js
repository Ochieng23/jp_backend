 ...
 *
 * @param {object} data - { full_name, date_of_birth, nationality, email, password_hash, phone?, unhcr_id? }
 * @returns {Promise<object>} Public holder object (no password_hash)
 */
export async function createHolder(data) {
  const holder = await PassportHolder.create(data);
  return holder.toPublic();
}
...
export async function findHolderByEmail(email) {
  return PassportHolder.findOne({ email: email.toLowerCase() })
...
export async function findHolderById(id) {
  return PassportHolder.findById(id).lean();
}

/**
 * Update allowed profile fields. Does NOT allow changing email or password.
 * @param {string} id
 * @param {object} data - { full_name?, phone?, nationality?, date_of_birth?, avatar_key? }
 * @returns {Promise<object|null>} Updated holder
 */
export async function updateHolder(id, data) {
  const allowed = {};
  for (const f of ['full_name', 'phone', 'nationality', 'date_of_birth', 'avatar_key', 'bio', 'unhcr_id']) {
    if (data[f] !== undefined) allowed[f] = data[f];
  }
  return PassportHolder.findByIdAndUpdate(id, allowed, {
    new: true,
    runValidators: true,
  }).lean();
}
